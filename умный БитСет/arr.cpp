...
CSet::CSet(size_t n) : size_(n), set_(make_unique<uint64_t[]>(Capacity(size_))) {
    for(size_t i = 0; i < Capacity(size_); i++) {
        set_[i] = 0;
    }
}

size_t CSet::Capacity(size_t size)const
{
...
CSet::CSet(const CSet& other):size_(other.size_),set_(make_unique<uint64_t[]>(Capacity(size_)))
{
	for(size_t i=0; i<Capacity(size_); i++)
	{
		set_[i]=other.set_[i];
...
CSet::CSet(CSet&& other):size_(other.size_),set_(move(other.set_))
{
	other.set_=nullptr;
}*/
...
CSet& CSet::operator=(const CSet& other) {
    if(this != &other) {
        size_ = other.size_;
        set_ = make_unique<uint64_t[]>(Capacity(size_));
        for (size_t i = 0; i < Capacity(size_); i++) {
            set_[i] = other.set_[i];
        }
    }
...
CSet& CSet::operator|=(const CSet& other)//побитовое или
{
	...
	{
		unique_ptr<uint64_t[]> new_set=make_unique<uint64_t[]>(Capacity(other.size_));
		for(size_t i=0; i<Capacity(size_);i++)
		{
			new_set[i]=set_[i];
		}
